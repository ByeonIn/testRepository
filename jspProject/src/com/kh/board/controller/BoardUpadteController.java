...
/**
 * Servlet implementation class BoardUpadteController
 */
@WebServlet("/update.bo")
public class BoardUpadteController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public BoardUpadteController() {
        super();
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			if(multiRequest.getOriginalFileName("reUpfile")!=null) {
				...
				if(multiRequest.getParameter("fileNo")!=null) {
					...
					//update attachment
					//기존 파일번호를 (식별자)를 이용하여 데이터 변경하기
					at.setFileNo(Integer.parseInt(multiRequest.getParameter("fileNo")));
					
					//기존에 첨부파일 삭제하기.
					new File(savePath+"/"+multiRequest.getOriginalFileName("originFileName")).delete();
