...
public class BoardInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * form에서 multipart/form-date 형식으로 전송했기 때문에 기존 request에서 parameter영역에서
		...
		*/
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			 ...
			 * 
			 * 매개변수 생성자로 생성
			 * MultipartRequest multRequest = new MultipartRequest(request객체,저장할 폴더 경로,용량제한,인코딩값,파일명 수정객체);
			 * 
			 * 위 구문 한줄이 실행되는 순간 지정하 경로에 파일이 업로드 된다.
			 * 사용자가 올린 파일명은 그대로 해당 폴더에 업로드 하지 않는게 일반적이다 - 같은 파일명이 있을 경우 덮어씌워질 경우도 있고
			 * 한글/특수문자가 포함된 파일명 경우는 업로드 되는 서버에 따라 오류를 발생시킬 여지가 있기때문.
			 * 
...
//			MultipartRequest multiRequest = new MultipartRequest(request, savePath,maxSize,"UTF-8",new DefaultFileRenamePolicy());
			...
			//3.DB에 저장할 정보들 추출하기 
			//-카테고리 번호,제목,내용,사용자번호 추출하여 board 테이블에 insert하기
			...
			if(result>0) {
				...
				if(at!=null) {//넘어온 파일이 있어서 객체가 생성됐다
